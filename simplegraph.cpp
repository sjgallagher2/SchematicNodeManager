...
Vec<int> SimpleGraph::get_node_ids()
{
    Vec<int> vids(vertices.size());
...
Vertex& SimpleGraph::get_node(int id)
{
    for(auto& v : vertices)
    {
...
int SimpleGraph::add_node(Coordinate2 p, bool traverse)
{
    Vertex vn(idpool.get(), p);
    // First check if this vertex is already present
    for(auto& other : vertices)
    {
        if(other == vn) {
            idpool.add_back(vn.get_id());
            return other.get_id();
        }
    }
    ...
    adjacent.insert(pair(vn.get_id(),Vec<int>{}));
...
void SimpleGraph::delete_node(int id, bool traverse)
{
    try {  // Checks that id is in graph, throws invalid_argument otherwise
        ...
        Vec<int> adj_id = adjacent.at(id);  // Copy this so we don't modify while looping
...
bool SimpleGraph::are_nodes_reachable(int id1, int id2, bool force_traverse)
{
    ...
    if(vertex_tree_id.empty()) traverse_graph();
    try {
        return vertex_tree_id.at(id1) == vertex_tree_id.at(id2);
    } catch (std::out_of_range) {
...
{
    /* Set Up */
    using CItr = Vec<Vertex>::const_iterator;  // Iterators are safe, underlying Vec won't change
    ...
    vertex_tree_id.clear();      // remap vertex id -> tree id
     ...
     * Mark the first vertex as visited, check if it has any adjacent vertices
     ...
     */
    CItr current_itr = vertices.cbegin();
    CItr next_itr = vertices.cend();
    int current_id = 0;              // Current id, for convenience
    while(current_itr != vertices.cend())
    {
        // 1. Initialize
        current_id = vert_itr_id[current_itr];
        next_itr = vertices.cend();
        Vec<int> current_adjs = adjacent[current_id];
        ...
        if(!visited[current_id])
        {
            ...
            dfs_id++;
            vertex_tree_id[current_id] = tree_id;
        }
...
bool SimpleGraph::are_nodes_adjacent(int id1,int id2)
{
    ...
    if(!(any_of(vertices,MatchingId{id2}))) { throw std::invalid_argument("Supplied id2 is not in the graph."); }

    try
    {
        Vec<int>& id1_adj = adjacent.at(id1);
...
Vec<int> SimpleGraph::get_reachable_nodes(int id, bool force_traverse)
{
    ...
    // Check if id exists
    if(!(any_of(vertices,MatchingId{id}))) { throw std::invalid_argument("Supplied id is not in the graph."); }

    int tree_id = -1;
    try{
        tree_id = vertex_tree_id.at(id);
    }catch(std::out_of_range){
